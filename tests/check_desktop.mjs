  ...
  if (process.argv.includes('--preview')) {
    ...
    await writeFile(preview, `
      ...
      plugin.register({register:c=>{if(c.area==='routes')page=c.render},onDispose:()=>{},i18n:{register:setBundles,t:translate},os:{openExternal:async()=>true},rest:async(path,options)=>{
        ...
        if(path.startsWith('/activity?')){const year=new URL('http://fixture'+path).searchParams.get('year');return {days:[{date:year+'-01-01',responses:2,cost_usd:0.12},{date:year+'-01-02',responses:4,cost_usd:0.24},{date:year+'-01-03',responses:38,cost_usd:13.786149}]}};
    ...
    export default (async () => {
      ...
      const ctx = {register: c => contributions.push(c), onDispose: fn => disposers.push(fn), i18n:{register:setBundles,t:translate},os:{openExternal:async()=>true},rest:async(path,options) => {
        ...
        if (path.startsWith('/activity?')) {
          ...
          return {days:[{date:year+'-01-01',responses:2,cost_usd:0.12},{date:year+'-01-02',responses:4,cost_usd:0.24},{date:year+'-01-03',responses:38,cost_usd:13.786149}]};
        }
      ...
      assert(day.title.includes('≈0.17%'));
      ...
      assert(document.querySelector('[data-date="'+(currentYear-1)+'-01-03"]').title.includes('≈20%'));
      fireEvent.click(await screen.findByRole('tab',{name:/Sessions/}));
      await screen.findByText('Fix login');
      assert.equal(screen.queryByRole('columnheader',{name:'Tokens'}), null);
      assert.equal(screen.queryByText('1k in · 200 out'), null);
      assert(screen.getByRole('columnheader',{name:'Est. use % · Pro 5x/week'}));
      assert(screen.getByRole('cell',{name:'≈0.17%'}));
      ...
      assert(screen.getByText('Estimated Pro 5x weekly use'));
      assert.equal(screen.getAllByText('≈0.17%').length, 2);
      assert(screen.getByText(/calibrated from observed usage\. Not measured quota/));
